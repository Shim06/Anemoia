...
Mapper_185::Mapper_185(uint8_t PRGBanks, uint8_t CHRBanks) : Mapper(PRGBanks, CHRBanks)
{
}
...
bool Mapper_185::cpuMapRead(uint16_t addr, uint32_t& mapped_addr, uint8_t& data)
{
...
bool Mapper_185::ppuMapRead(uint16_t addr, uint32_t& mapped_addr)
{
	...
	if (addr == 0x2007 && garbage_read_count < 2)
	{
		garbage_read_count++;
		return true;
	}
...
bool Mapper_185::ppuMapWrite(uint16_t addr, uint32_t& mapped_addr)
{
	return false;
}
