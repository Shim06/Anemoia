...
int App::init(SDL_Window*& init_window, SDL_Renderer *& init_renderer)
{
    window = init_window;
    ...
    games_list = getNESFiles((current_directory + "games"));
    ...
    font_size_24 = io->Fonts->AddFontFromFileTTF((current_directory + "fonts\\roboto\\roboto-regular.ttf").c_str(), 24.0f * 1.5f, &font_cfg);
    bold_font = io->Fonts->AddFontFromFileTTF((current_directory + "fonts\\roboto\\roboto-bold.ttf").c_str(), 18.0f * 1.5f, &font_cfg);
    for (wchar_t i = 0x2150; i <= 0x218F; i++) // UTF-8 Number Forms
        builder.AddChar(i);
    builder.AddRanges(io->Fonts->GetGlyphRangesJapanese());
    builder.BuildRanges(&ranges);
    japanese_font = io->Fonts->AddFontFromFileTTF((current_directory + "fonts\\noto_sans_jp\\notosansjp-bold.ttf").c_str(),
        20.0f * 1.5f, &font_cfg, ranges.Data);

    loadSettings(current_directory + "system\\settings.json");

    // Load image previews for ROMs
    missing_preview = SDL_CreateTexture(renderer, SDL_PIXELFORMAT_RGB24, SDL_TEXTUREACCESS_STREAMING, 256, 240);
    std::ifstream ifs;
    ...
    controller_image = SDL_CreateTextureFromSurface(renderer, controller_surface);
    if (controller_surface == NULL)
    {
        printf("Unable to load image! SDL_image Error: %s\n", IMG_GetError());
        return 1;
...
void App::drawMenuBar(bool& done)
{
...
void App::handleSDLEvents(bool& done)
{
    SDL_Event event;
...
std::string App::loadROM(HWND hwnd)
{
    OPENFILENAMEA ofn;
    CHAR szFile[260] = { 0 };
...
std::string App::saveState(HWND hwnd)
{
    OPENFILENAMEA ofn;
    CHAR szFile[260] = { 0 };
    ZeroMemory(&ofn, sizeof(OPENFILENAME));
...
std::string App::loadState(HWND hwnd)
{
    OPENFILENAMEA ofn;
    CHAR szFile[260] = { 0 };
    ZeroMemory(&ofn, sizeof(OPENFILENAME));
...
void App::loadSettings(const std::string& file_path)
{
...
void App::handleKeybindRemap(int type)
{
    if (!keybind_selected)
        return;
...
void App::setVolume(Emulator& emu)
{
...
void App::saveImage(const char* file_path, SDL_Renderer* renderer, uint8_t* frame_buffer)
{
...
void App::updatePlayTime(const std::string& folder_path, int duration)
{
...
void App::updateLastPlayedTime(const std::string& folder_path, game& game)
{
...
void App::savePreview(const std::string& path, Emulator& emu)
{
